...
class UserController extends Controller
{
    ...
    {
        //
        return view('receptionist.users.index');
    }
    ...
    public function create()
    {
        ...
        return view('receptionist.users.create', compact('role'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('receptionist.users.index');
    }
    ...
    {
        //
        return view('receptionist.users.show', compact('user'));
    }
    ...
    public function edit(User $user)
    {
        ...
        return view('receptionist.users.edit', compact('user', 'roles'));
    }
...
public function update(Request $request, User $user) // Usamos Route Model Binding para User
    {
        // 🚨 Restringir modificación del receptionist principal
        if ($user->id === 1) {
             session()->flash('swal',
            [
                'icon' => 'error',
                'title' => 'ERROR',
                'text' => 'No se puede modificar al usuario receptionististrador principal'
            ]);
            return redirect()->route('receptionist.users.index');
        }
        ...
        if (!$hasChanges) {
            ...
            return redirect()->route('receptionist.users.edit', $user);
        }
    ...
    {
        // Restringir eliminar el primer usuario (receptionist principal)
        if ($user->id === 1) {
            session()->flash('swal',
            [
                'icon' => 'error',
                'title' => 'ERROR',
                'text' => 'No se puede eliminar al usuario receptionististrador principal'
            ]);
